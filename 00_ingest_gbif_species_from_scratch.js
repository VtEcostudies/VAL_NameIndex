/*
  ...
  File: 08_ingest_gbif_species_from_scratch.js

  Purpose: Incorporate periodic updates to VT Species backbone, usually when we
  update VAL DE occurrences quarterly.

  Details:
    - bounce incoming taxonKey against new_species first
    - if key is found in new_species, ignore
    - if key is NOT found, use that key on GBIF taxon api to key full result
    - use that result to INSERT new taxon into new_species.
...
var fileName = 'gbif_species_update_w_loc';
var fileExt = '.tsv';
...
var errCount = 0; //count records inserted
...
function getColumns() {
  ...
  return pgUtil.getColumns("new_species", staticColumns);
}
...
function getGbifTaxon(key, idx) {
  ...
  return new Promise(async (resolve, reject) => {
    await Request.get(parms, (err, res, body) => {
      if (err) {
...
/*
  Get VAL species from new_species table by gbif taxonKey
*/
async function getValTaxon(gbif, idx) {

  var sql_select = `select * from new_species where "taxonId"=$1`;

  return new Promise(async (resolve, reject) => {
    await query(sql_select, [gbif.taxonKey])
      .then(res => {
        res.gbif = gbif;
        res.idx = idx;
        resolve(res);
...
return new Promise(async (resolve, reject) => {
  try { //wrap whole thing in trap to deal with random errors during development...
    ...
    /*
      ...
      select count(*) from new_species where LOWER("taxonomicStatus") like '%doubt%';
     */
    ...
    var sql_insert = `insert into new_species (${qryColumns.named}) values (${qryColumns.numbered}) returning "taxonId"`;
...
async function updateValTaxon(val, idx) {
  ...
  var sql_update = `update new_species set (${qryColumns.named}) = (${qryColumns.numbered}) where "gbifId"=$1`;

  return new Promise(async (resolve, reject) => {
    await query(sql_update, qryColumns.values)
      .then(res => {
        res.val = val;
        res.idx = idx;
        resolve(res);
